...
"""
Read raw accelerometer and gyroscope data from an MPU6050 IMU
on a moteus controller configured with Type 6 (mpu6050 mode).

This is the MPU6050 counterpart of read_accel_gyro.py (which targets the
LSM6DSV16X in Type 5 mode). The firmware exposes MPU6050 data through the
SAME CAN registers as the LSM6DSV16X, so the only real differences here are:
  1. the gyroscope scale factor (the MPU6050 has different sensitivity), and
  2. the configuration commands (type 6, I2C address 104 = 0x68).

This script is a host-side monitor only -- it runs on your PC and does not
affect the firmware or the firmware build in any way.

It continuously reads 6 values:
...
# - MPU6050 configured for ±16g full scale range (ACCEL_CONFIG = 0x18)
# - 16-bit output: -32768 to +32767 counts
# - Sensitivity at ±16g = 2048 LSB/g -> 1 / 2048 = 0.000488 g/count
# - Convert to SI units: 0.000488 g/count × 9.80665 m/s²/g = 0.004786 m/s²/count
# (Same as the LSM6DSV16X at ±16g.)
ACCEL_SCALE = 0.000488 * 9.80665  # m/s² per LSB (at ±16g range)

# Gyroscope scale factor:
# - MPU6050 configured for ±2000 degrees/second full scale (GYRO_CONFIG = 0x18)
# - From datasheet: sensitivity = 16.4 LSB per deg/s
# - So each count = 1 / 16.4 = 0.06098 deg/s
# NOTE: this DIFFERS from the LSM6DSV16X (which is 0.07 deg/s per count).
GYRO_SCALE = 1.0 / 16.4  # degrees/second per LSB (at ±2000 dps range)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description='Read accelerometer and gyroscope from moteus controller (MPU6050)'
    )
    parser.add_argument(
        '--target',
        type=int,
    ...
    # These are the same registers the LSM6DSV16X raw mode uses; the MPU6050
    # firmware driver populates them identically.
    qr._extra = {
        # Accelerometer data (reuses quaternion registers in raw mode)
    ...
    print(f"Make sure controller is configured with type 6 (mpu6050)")
